...
<main class="index-create">
    ...
    <form action="app/update_note.php" method="POST">
        ...
        <input id="note-title" type="text" name="title" placeholder="Titulo da anotação" value="<?= $note['title']; ?>">
        <label for="note-body">Corpo da anotação: </label>
        <textarea id="note-body" name="body" placeholder="Descreva sua anotação"><?= $note['body']; ?></textarea>
